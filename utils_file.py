...
def sanitize_filename(text, max_length=80):
    """
    Limpa um texto para que seja um nome de arquivo válido.
    Remove caracteres inválidos, substitui espaços e limita o comprimento.
    """
    if not text:
        return "audio_sem_texto"
    
    # Remove caracteres inválidos para nomes de arquivo no Windows e Unix
    ...
    # Remove underscores do final, se houver
    sanitized = sanitized.rstrip('_')

    # Se após tudo isso o nome ficar vazio, retorna um padrão
    if not sanitized:
        return "audio"
...
async def merge_audio_files(output_folder, srt_file_path):
    """
    Mescla segmentos de áudio de forma universal e utiliza um arquivo WAV intermediário
    para garantir que os metadados de duração do MP3 final sejam 100% corretos.
    """
    ...
    # Etapa 1: Juntar todos os segmentos na memória (código anterior, já está correto)
    with tqdm(total=len(subs), desc=f"Mesclando áudios para {base_name}", unit="segmento") as pbar:
        for sub in subs:
            ...
            audio_segment = None
            possible_files = [
                output_folder / f"{sub.index:04d}.mp3", output_folder / f"{sub.index:04d}.wav",
                output_folder / f"{sub.index:02d}.mp3", output_folder / f"{sub.index:02d}.wav"
            ]
            
            found_file = next((f for f in possible_files if f.exists() and f.stat().st_size > 1024), None)
            ...
            if found_file:
                if found_file.suffix == '.mp3': audio_segment = AudioSegment.from_mp3(str(found_file))
                elif found_file.suffix == '.wav': audio_segment = AudioSegment.from_wav(str(found_file))
    ...
    # --- Etapa 2: Exportação robusta com WAV intermediário ---
    srt_output_dir = Path("output/srt_output")
    srt_output_dir.mkdir(parents=True, exist_ok=True)
    
    final_mp3_path = srt_output_dir / f"{base_name}_final.mp3"
    temp_wav_path = srt_output_dir / f"{base_name}_temp_intermediario.wav"

    try:
        # 2.1. Exporta a montagem para um WAV temporário. Este arquivo terá a duração correta.
        print("\nCriando arquivo WAV intermediário para garantir precisão dos metadados...")
        final_audio.export(str(temp_wav_path), format="wav")

        # 2.2. Carrega o áudio a partir do WAV perfeito.
        print("Convertendo para MP3 com metadados corretos...")
        audio_from_wav = AudioSegment.from_wav(str(temp_wav_path))

        # 2.3. Exporta para MP3. A conversão de WAV para MP3 cria um cabeçalho limpo.
        audio_from_wav.export(str(final_mp3_path), format="mp3", bitrate="192k")
        
        print(f"Metadados corrigidos com sucesso. Áudio final salvo em: {final_mp3_path}")

    finally:
        # 2.4. Garante que o arquivo WAV temporário seja sempre deletado.
