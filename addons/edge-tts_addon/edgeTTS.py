...
from utils_file import remove_silence, timetoms, merge_audio_files, adjust_audio_speed
from get_voices import get_voices

# --- Funções de Gerenciamento de Voz ---
def load_voices():
    """
    Carrega as vozes do arquivo 'voices.json'. 
    Se o arquivo não existir, executa get_voices() para criá-lo.
    """
    if not os.path.exists('voices.json'):
        print("Arquivo 'voices.json' não encontrado. Executando get_voices() para criá-lo...")
        try:
            get_voices()
            print("'voices.json' criado com sucesso.")
        except Exception as e:
            print(f"Erro ao tentar criar 'voices.json': {e}")
            # Retorna um dicionário vazio para não quebrar a UI
            return {}
    ...
    try:
        ...
        # Se ainda falhar, retorna vazio para a UI não quebrar
        print("AVISO: Falha ao carregar 'voices.json'. A lista de vozes do Edge-TTS estará vazia.")
        return {}
